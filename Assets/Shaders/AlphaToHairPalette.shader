Shader "Unlit/AlphaToHairPalette"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float paletteU = (((alphaIndex < 0.06) ? 0 : (alphaIndex < 0.2) ? 0 : (alphaIndex < 0.3) ? 1 : (alphaIndex < 0.8) ? 2 : 1) + 0.5) * _PaletteTex_TexelSize.x;
                float paletteV = (((alphaIndex < 0.06) ? 0 : (alphaIndex < 0.8) ? _IndexInt +1 : 0) + 0.5) * _PaletteTex_TexelSize.y;
